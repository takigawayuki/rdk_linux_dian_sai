...
from Algorithm.CenterKalman import CenterKalman
...
# 卡尔曼滤波参数
# ============================================================
KF_DT       = 1.0 / 110.0   # 算法帧周期估计值（CenterGet 大约 110 FPS）
KF_R        = 4.0           # 测量噪声：越小越信测量
KF_Q        = 20.0          # 过程噪声：越大越敢跟随快动
KF_MAX_LOST = 15            # 最多用几帧纯预测；超过就 reset
...
# 显示线程用的最新结果
latest_center     = None     # 最终用的靶心（KF 滤波后或纯预测）
latest_center_raw = None     # CenterGet 直接返回的原始测量
latest_predicted  = False    # True = 当前 center 是 KF 纯预测出来的
...
# ============================================================
def thread_algo():
    global latest_center, latest_center_raw, latest_predicted
    global latest_laser_pt, latest_distance
    ...
    # 卡尔曼滤波器
    kf = CenterKalman(dt=KF_DT, R=KF_R, Q=KF_Q)
    lost_count = 0
    last_smooth_area = 0.0   # 跟丢时复用的最近一次面积，用于距离/激光点估算
    ...
    while running:
        ...
        if result is None:
            lost_count += 1
            if kf.initialized and lost_count <= KF_MAX_LOST:
                pred = kf.predict_only()
                if pred is not None:
                    distance = estimate_distance(last_smooth_area)
                    laser_pt = calc_laser_point(distance) if distance > 0 else None
                    with state_lock:
                        latest_center     = pred
                        latest_center_raw = None      # 没有测量
                        latest_predicted  = True
                        latest_laser_pt   = laser_pt
                        latest_distance   = distance
                    if result_queue.full():
                        try: result_queue.get_nowait()
                        except: pass
                    result_queue.put_nowait((pred, laser_pt, distance))
                    continue

            # 没初始化 / 跟丢太久：清状态、放弃
            if lost_count > KF_MAX_LOST:
                kf.reset()
            with state_lock:
                latest_center     = None
                latest_center_raw = None
        ...
        # ----------- 识别成功：测量 + 滤波 -----------
        center_raw, area = result
        lost_count = 0
        center = kf.update(center_raw)
    ...
    try:
        while True:
            ...
            if center_raw is not None:
                cv2.circle(frame, center_raw, 4, (160, 160, 160), 1)
            ...
            if center is not None:
                if predicted:
                    ...
                    cv2.putText(frame, "PRED", (center[0] + 8, center[1] + 4),
